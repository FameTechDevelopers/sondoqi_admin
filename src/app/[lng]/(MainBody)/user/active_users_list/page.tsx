...
const ActiveUsersList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/users/getActiveUsersList`, {
        // params: {
        //   page,
        // },
        headers: {
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery(`activeUsersList_${page}${getUpdate}`, fetchData);
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.get(`${BASE_URL}/users/deleteUser`, {
        params: {
          user_id: id,
